...
protected:
	...
	void com_set_bit(int num_bit);  // Helper function for galil command "SB"
	void com_clear_bit(int num_bit); // Helper function for galil command "CB"
	void com_query_digital(int num_bit); // Helper function for galil command "MG @IN"
	void com_query_analogue(int channel); // Helper function for galil command "MG @AN"

	bool default_use; //checks if the default constructor has been used or not.
	bool command_success; //checks if the write command to the galil was successful
};
